...
## 2026-02-19
- Устранен ключевой сценарий `Демо: недоступно` для WOW-момента в окне ротации/архивации MyArena: бот теперь запускает retry не только при `map_mismatch`, но и при `no_demo_found` (до 35 сек, шаг 5 сек).
- В `HltvDemoResolver` добавлены диагностические логи по кандидатам демо: для HLTV фиксируются `demo_path/demo_map/map_expected`, для FTP — число найденных `*.dem`, число map-compatible кандидатов и итоговый выбранный файл.
- В `MomentState` сравнение карт переведено на нормализованное имя (`de_dust2_2x2 -> de_dust2`), чтобы служебные `info`-снапшоты не делали ложные reset сессии WOW.
- Добавлены тесты `tests/test_wow_moments.py` на сценарии с вариантами имени карты (`de_dust2` vs `de_dust2_2x2`) и сохранение агрегации голосов в одном кластере.
- Обновлены `README.md` и `docs/PROJECT_DOC_RU.md`: зафиксирован retry-сценарий при `no_demo_found` в MyArena.
- Добавлен новый игровой поток `LOL`/кринж-моментов: триггеры `lol`, `/lol`, `!lol` в AMXX отправляют `moment_vote` с `moment_kind=lol` (WOW остаётся через `omg`, `/omg`, `!omg` с `moment_kind=wow`).
- В боте агрегация WOW и LOL разделена по `moment_kind`, чтобы разные типы моментов не склеивались в один кластер.
- В Discord-постах добавлено визуальное различие при скролле: `🔥 WOW-момент` и `🤡 Кринж-момент`; для LOL-счётчика используются эмодзи `🤡` вместо `⭐`.
- Обновлены `README.md`, `docs/PROJECT_DOC_RU.md`, `docs/COMMANDS_REFERENCE_RU.md` под новые триггеры и формат отображения.

## 2026-02-18
- Усилено логирование WOW demo resolve при коллизиях распознавания текущей демки: в логах теперь фиксируются source (`hltv`/`ftp`/`cache`), expected/found map, demo path, attempted sources, reason и шаги retry.
- Для кейсов `Демо: недоступно` добавлен финальный лог с причиной (`resolver_disabled` / `no_demo_found` / `map_mismatch`) и деталями последней попытки.
- Поднята версия проекта в `VERSION` до `0.5.1` для доставки изменений через `updater.sh`.
- В WOW-посте изменен вывод звезд: вместо `xN` теперь эмодзи `⭐` (до 10 символов), при превышении — компактный формат `⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐ xN`.
- Добавлен helper `format_stars_emoji()` и тесты на рендер звезд для значений 1/7/10/11/25.
