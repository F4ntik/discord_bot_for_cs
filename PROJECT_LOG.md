...
- Исправлена деградация RCON-состояния: при ошибке `CSRCON.exec()` теперь принудительно закрывается сокет и сбрасывается `connected=False`, чтобы бот не оставался в ложном состоянии «подключен, но команды не уходят».
- В декоратор `require_connection` добавлен авто-reconnect перед выполнением команд, чтобы Discord -> CS восстанавливался без ручного `/connect_to_cs` после кратковременного обрыва.
- Добавлено дополнительное логирование отправки trigger-команды `ultrahc_ds_push_maps` (mode/request_id/response) для диагностики `/server_maps*`.
- Версия бота повышена до `0.4.11` (`VERSION`) в совместимой линии `B=4`.
- По инциденту «команды `/rcon` не доходят до сервера» откатан `rehlds/rcon.py` к стабильной версии из `8b74e2f` (до изменения multi-packet чтения ответов из `fddc86e`).
- Синхронно откатан регрессионный тест `tests/test_rcon_parsing.py` к тому же стабильному снапшоту.
- Версия бота повышена до `0.4.12` (`VERSION`) в совместимой линии `B=4`.
- Для feature-ветки с внедрением webhook snapshot карт открыта новая линия совместимости `B=5`.
- Версии в этой ветке переведены на `0.5.*`: бот `0.5.1` (`VERSION`), AMX-плагин `0.5.1` (`PLUGIN_VERSION`).
