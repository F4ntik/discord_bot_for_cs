...
- Исправлена деградация RCON в `main`: при ошибке `CSRCON.exec()` теперь принудительно закрывается сокет и сбрасывается `connected=False`, чтобы бот не зависал в ложном состоянии «подключен».
- В `require_connection` добавлен авто-reconnect перед выполнением команд, поэтому `/rcon` и Discord -> CS чат восстанавливаются без ручного `/connect_to_cs` после краткого обрыва.
- В `/rcon` добавлено отдельное лог-сообщение перед отправкой команды для быстрой диагностики (`executing RCON command: ...`).
