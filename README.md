...
- Статус CS в Discord обновляется push-вебхуками из AMX-плагина (события + периодический heartbeat), без постоянного RCON-поллинга.
- Команда `/map_install` работает в прод-режиме: принимает `.bsp/.zip`, загружает карту в `maps/` через FTP/FTPS и может сразу добавить её в ротацию.
