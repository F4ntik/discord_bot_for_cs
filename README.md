...
## WOW moments (экспериментально)
- В проект добавлен realtime-поток WOW-моментов из AMXX в Discord.
- Игровой триггер: `omg`, `/omg`, `!omg` -> меню выбора игрока.
- Бот агрегирует голоса в окне `WOW_MOMENT_WINDOW_SEC` и обновляет один пост в `MOMENTS_CHANNEL_ID`.
- Для демо-ссылки используется HLTV RCON (`status` -> `Recording to ...`) или FTP fallback (`WOW_DEMO_FTP_*`).
- Если карта события и карта в demo не совпадают (редкое окно после смены карты), бот ждет до 35 секунд и ретраит поиск, чтобы не отправлять ложную ссылку.
- Если в момент публикации нет подходящей demo (`no_demo_found`, например во время архивирования/ротации на MyArena), бот также делает ретраи до 35 секунд с шагом 5 секунд.
- В WOW-посте используется таймкод `~MM:SS` от старта карты (`map_elapsed_sec` из `get_gametime()`), а не только `timeleft`.
- Формат WOW-поста русифицирован, добавлена строка `Кто отметил`.
- Звезды отображаются эмодзи `⭐` (до 10), при большем числе показывается компактный суффикс `xN`.
...
- Для теста ветки: `./updater.sh --branch feature/wow-moments`.
- Для отката: `./updater.sh --branch main`.
- `updater.sh` не перезаписывает `config.py`, а только добавляет недостающие ключи из шаблона.
- Новые WOW-параметры в `config.py` помечены как `[EXPERIMENTAL]`: их можно заполнить один раз, и они сохранятся при переходах `ветка <-> main`.
- `updater.sh` не обновляет `amxmodx_plugin/`, поэтому актуальный `ultrahc_discord.amxx` выкладывается отдельно.
