...
- Для устойчивости при кратковременных сетевых обрывах бот автоматически пытается переподключиться к RCON перед выполнением команды; при ошибке сокета состояние подключения принудительно сбрасывается и восстанавливается на следующей команде.
