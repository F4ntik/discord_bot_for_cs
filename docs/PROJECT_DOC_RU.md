...
## Команда `/map_install`
Команда предназначена для прод-установки карт через Discord: загрузка вложения, доставка в `maps/` по FTP/FTPS и (опционально) регистрация в ротации.

- **Параметры:**
  - `file` — вложение с картой (`.bsp` или `.zip`).
  - `map_name` — необязательное имя карты без расширения.
  - `min_players` и `max_players` — числовые параметры ротации (используются при `add_to_rotation=true`).
  - `add_to_rotation` — флаг добавления карты в ротацию (БД/Redis + reload map list).
  - `priority` — приоритет карты в ротации (по умолчанию `100`).
- **Результат выполнения:**
  - вложение проходит валидацию формата: принимаются `.bsp` и `.zip` (в zip допускается ровно один `.bsp`);
  - карта доставляется на игровой сервер в `maps/` через FTP/FTPS;
  - при `add_to_rotation=true` карта добавляется в БД/Redis, если записи ещё нет, затем выполняется `ultrahc_ds_reload_map_list`;
...
- **Необходимые настройки в `config.py`:**
  - `MAP_DEPLOY_PROTOCOL`, `MAP_FTP_HOST`, `MAP_FTP_PORT`, `MAP_FTP_USER`, `MAP_FTP_PASSWORD`;
  - `MAP_REMOTE_MAPS_DIR`, `MAP_FTP_PASSIVE`, `MAP_FTP_USE_TLS`, `MAP_FTP_TIMEOUT_SEC`;
  - `MAP_INSTALL_MAX_FILE_MB`, `MAP_INSTALL_SYNC_TIMEOUT_SEC`.
