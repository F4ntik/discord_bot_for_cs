...
3. Установите зависимости:
   ...
   ```
<!-- PAD:................................................................................................................................................................................................................ -->
Примечание: параметр `CS_INFO_WEBHOOK_MAX_MISSES` определяет, сколько раз подряд допускается не получить webhook `type=info` перед переподключением (по умолчанию 3).
...
Статус в информационном канале обновляется циклом: бот по таймеру выполняет RCON-команду `ultrahc_ds_get_info`, а сервер в ответ отправляет HTTP webhook `type=info` на эндпоинт `/webhook`.

Если webhook не приходит в течение `CS_INFO_WEBHOOK_TIMEOUT` секунд, бот считает связь деградировавшей, инициирует переподключение к CS-серверу и продолжает попытки автоматически (с интервалом `CS_RECONNECT_INTERVAL`). Команду `/connect_to_cs` можно использовать как ручной «форс» переподключения.
Ответы RCON на ultrahc_ds_get_info и ultrahc_ds_send_msg проверяются на типовые ошибки (Unknown command, Bad rcon_password). При их появлении в лог пишется ошибка, соединение помечается отключённым и перезапускается коннектор, чтобы не зависать в некорректном состоянии.
