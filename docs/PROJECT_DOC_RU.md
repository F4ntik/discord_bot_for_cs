...
Статус в информационном канале обновляется циклом: бот по таймеру выполняет RCON-команду ultrahc_ds_get_info (отдельное RCON-подключение на каждый цикл), а сервер в ответ отправляет HTTP webhook 	ype=info на эндпоинт /webhook.

Если webhook не приходит в течение CS_INFO_WEBHOOK_TIMEOUT секунд, событие фиксируется в логах и статус ждёт следующий цикл: бот отправит новый RCON-запрос без остановки задачи. Для ручной проверки связи можно вызвать /connect_to_cs.
