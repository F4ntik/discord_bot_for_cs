...
| `/map_install` | Администрирование CS | `manage_messages` | Загружает карту, отправляет на FTP/FTPS и опционально добавляет в ротацию. |
...
- **Назначение:** загружает файл карты через Discord, доставляет его в `maps/` по FTP/FTPS и, при необходимости, добавляет карту в ротацию.
- **Права:** `manage_messages`.
- **Параметры:**
...
- **Поведение:**
  - `.bsp` отправляется напрямую, `.zip` должен содержать ровно один `.bsp`;
  - карта загружается в удалённую папку `maps/` через FTP/FTPS;
  - при `add_to_rotation=true` бот добавляет карту в ротацию, если её нет в БД; при дубликате шаг БД пропускается с предупреждением;
  - если операция длится дольше лимита, команда автоматически переводится в фоновый режим и отправляет итог отдельным сообщением.
